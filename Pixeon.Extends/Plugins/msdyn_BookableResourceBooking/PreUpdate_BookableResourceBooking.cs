...
{
    public class PreUpdate_BookableResourceBooking : PluginBase
    {
        public PreUpdate_BookableResourceBooking() : base(typeof(PreUpdate_BookableResourceBooking)) { }
        ...
        protected void ValidateUpdate(BookableResourceBooking target, BookableResourceBooking preImage, msdyn_resourcerequirementBusiness business, LocalPluginContext localContext)
        {
            if (target.BookingStatus != null)
                business.ValidateUpdate(localContext.PluginExecutionContext.UserId, preImage, target.BookingStatus.Id, target.Id);
