...
HolidayRequest.Functions = {
    ...
    cancelHolidayRequest: function (primaryControl) {
        // debugger;
        "use strict";
        if (primaryControl.data.entity.getEntityName() === HolidayRequest_LogicalName &&
            primaryControl.getAttribute("statuscode").getValue() === 1)
        {
            var entityFormOptions = {};
            entityFormOptions["entityName"] = "smt_cancel_holiday_request";
            ...
            /*
            // Set lookup column
            formParameters["preferredsystemuserid"] = primaryControl.data.entity.getId(); // ID of the user.
            formParameters["preferredsystemuseridname"] = primaryControl.getAttribute("smt_name").getValue(); // Name of the user.
            formParameters["preferredsystemuseridtype"] = HolidayRequest_LogicalName; // Table name.
    ...
    rejectHolidayRequestVisible: async function (primaryControl) {        
        "use strict";
        ...
        if (primaryControl.getAttribute("statuscode").getValue() === 100000000) {
            ...
            if (primaryControl.getAttribute("smt_lp_resource").getValue() !== null) {
                ...
                var idUnitOrg = await ExtendedHelp.Functions.retrieveUnitOrganization(idResource);
                // setTimeout(idUnitOrg, 2000);
                ...
                if (idUnitOrg !== undefined && idUnitOrg !== null) {
                    ...
                    var parameterManager =  await ExtendedHelp.Functions.retrieveParameterSmart(idUnitOrg._msdyn_organizationalunit_value, ExtendedHelp.Functions.ParametersName.ManagerParameterHoliday);
    ...
    retrieveBookableResource: async function () {
        "use strict";
        var result = await Xrm.WebApi.retrieveMultipleRecords("bookableresource", "?$select=bookableresourceid,name&$filter=_userid_value eq aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa").then(
            ...
            function (success) {
                if (success.confirmed) {
                    ...
                    if (primaryControl.getAttribute("smt_st_justification").getValue() !== null) {
                        ...
                        Xrm.Navigation.openAlertDialog("A justificativa precisa ser preenchida para continuar com o processo de reprovação.");
                    }
    ...
    regraBtnEnviar: async function (primaryControl) {
        //debugger;
        "use strict";
        ...
        var retrieveRecurso = async function (id) {
            var results = await Xrm.WebApi.retrieveRecord("bookableresource", id, "?$select=_userid_value").then(
                function success(result) {
                    return result;                    
                },
                function (error) {
        ...
        // debugger
        var recurso = await retrieveRecurso(idUsuarioRegistro);
        // debugger;
    ...
    rejectHoliday: async function (primaryControl) {
        "use strict";
        var showButton = false;
        ...
        if (primaryControl.getAttribute("statuscode").getValue() === 2) {
            ...
            if (primaryControl.getAttribute("smt_lp_resource").getValue() !== null) {
                ...
                if (idUnitOrg !== undefined && idUnitOrg !== null) {
                    ...
                    //Realizar a busca em parametros (tornar o mais generico possivel)
                    var parameterRH = await ExtendedHelp.Functions.retrieveParameterSmart(idUnitOrg._msdyn_organizationalunit_value, ExtendedHelp.Functions.ParametersName.RHParameterHoliday);
    ...
    createHolidayRequest: function (id, start, end) {
        "use strict";
        var record = {};
        record.smt_dt_start = new Date(start); // Date Time
        record.smt_dt_end = new Date(end); // Date Time
        record["smt_lp_resource@odata.bind"] = "/bookableresources("+id+")"; // Lookup
        record.smt_smt_pl_type = 100000001; // Choice

        Xrm.WebApi.createRecord("smt_holiday_request", record).then(
            function success(result) {
                var newId = result.id;
                // Xrm.Navigation.openAlertDialog(newId);
            },
            function (error) {
                Xrm.Navigation.openAlertDialog(error.message);
            }
